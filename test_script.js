    ...
    document.addEventListener("DOMContentLoaded", () => {
      ...
      const placeAllCardsRandomly = () => {
        ...
        allProdCards.forEach(card => {
          const cardWidth = card.getBoundingClientRect().width || (card.classList.contains('landscape-card') ? 224 : 177);
          const cardHeight = card.getBoundingClientRect().height || (card.classList.contains('landscape-card') ? 208 : 280);
          ...
          // Attempt 1: Try placing on the right side
          ...
          // Attempt 2: If no space on the right, use the entire screen
          if (!foundSpot) {
            minX = mainRect.left + 24;
            if (maxX > minX && maxY > minY) {
              for (let attempts = 0; attempts < 500; attempts++) {
                finalX = minX + Math.random() * (maxX - minX);
                finalY = minY + Math.random() * (maxY - minY);
      ...
      allProdCards.forEach(prodCard => {
        ...
        prodCard.addEventListener("mousedown", (e) => {
          const prefix = prodCard.dataset.linePrefix;
          ...
          const onMouseMove = (ev) => {
            ...
            const minX = mainRect.left + 24;
            const maxX = mainRect.right - cardRect.width - 24;
            const minY = mainRect.top + 24;
            const maxY = mainRect.bottom - cardRect.height - 24;
            ...
            if (newX > actualMaxX) newX = actualMaxX;
            if (newY > actualMaxY) newY = actualMaxY;
          ...
          const onMouseUp = () => {
            ...
            const prefix = prodCard.dataset.linePrefix;
            document.body.classList.remove('is-interacting-' + prefix);
          };
        ...
        const clampProdCard = () => {
          ...
          if (newX < minX) { newX = minX; adjusted = true; }
          if (newY < minY) { newY = minY; adjusted = true; }
          if (newX > actualMaxX) { newX = actualMaxX; adjusted = true; }
          if (newY > actualMaxY) { newY = actualMaxY; adjusted = true; }
        ...
        const updateDlPosition = () => {
          if (dynamicLabel && dynamicLabel.classList.contains('visible')) {
            const rect = prodCard.getBoundingClientRect();
            ...
            if (imgEl) {
              ...
              // Calculate bottom of label to be 20% above the image's bottom
